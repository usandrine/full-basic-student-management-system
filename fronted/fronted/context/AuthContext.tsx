...
interface AuthContextType {
  ...
  logout: () => void;
  requestPasswordReset: (email: string) => Promise<{ success: boolean; message: string }>; // NEW: Password reset function
  loading: boolean;
  error: string | null;
  clearError: () => void;
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // NEW: Function to send a password reset request to the backend API
  const requestPasswordReset = async (email: string) => {
    setLoading(true);
    setError(null);
    try {
      // Make a POST request to the new backend endpoint
  ...
  const value = { user, token, login, register, logout, loading, error, clearError, updateUser, requestPasswordReset };
